...
import {
  ...
  CONTRAINDICATIONS, RED_LINES, recommendRehab, suggestedPhase, morningWindow,
} from '../lib/rehab'
...
export default function Lesion() {
  ...
  return (
    <div className="px-4 pt-4 pb-8 space-y-4">
      <ScreenHeader title="Lesión" sub="Lumbar L2-L5 · sacralización L5 · rodilla dcha · readaptación" />
    ...
    <div className="card p-4">
      ...
      {show && (
        <div className="space-y-3 mt-3">
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'rgba(239,68,68,.06)', border: '1px solid rgba(239,68,68,.3)' }}>
            <div className="text-[11px] font-semibold uppercase" style={{ color: '#EF4444', letterSpacing: '0.06em' }}>
              🚨 Líneas rojas (guía de readaptación)
            </div>
            {RED_LINES.map(r => (
              <div key={r.title}>
                <div className="text-[12px] font-medium" style={{ color: 'var(--text)' }}>{r.title}</div>
                <div className="text-[11px] mt-0.5" style={{ color: 'var(--text-3)' }}>{r.detail}</div>
              </div>
            ))}
          </div>
