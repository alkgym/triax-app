...
const TAB_LABEL: Record<Tab, string> = { resumen: 'Resumen', gym: 'Gym', cardio: 'Cardio', peso: 'Peso' }
...
function ResumenTab() {
  ...
  async function exportBackup() {
    const [p, ex, pd, se, st, bm, pr, nu, ge, pl] = await Promise.all([
      db.profile.toArray(), db.exerciseTemplates.toArray(), db.planDays.toArray(),
    ...
    const blob = new Blob([JSON.stringify({ exportedAt: today, profile: p, exerciseTemplates: ex, planDays: pd, sessions: se, sets: st, bodyMetrics: bm, prs: pr, nutrition: nu, gear: ge, painLogs: pl }, null, 2)], { type: 'application/json' })
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `gym-backup-${today}.json`; a.click()
  }
