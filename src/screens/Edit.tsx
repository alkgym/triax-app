...
function DataPanel() {
  ...
  async function importData(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const restored = await restoreBackup(dump)
      setImportMsg(`✓ Backup restaurado (${restored.length} tablas) — ${dump.exportedAt ?? 'fecha desconocida'}`)
    } catch (err) {
      setImportMsg(`✕ ${err instanceof Error ? err.message : 'Error leyendo el archivo'}`)
    }
    e.target.value = ''
  }
